define(["jquery"],function($,subscribeFooter) {
	return {
		subscribeFooter: function() {
			$('#form_epaper1').on('submit', function(e) {
				e.preventDefault();
				var $this = $(this);
				...
				if (/(.+)@(.+){2,}\.(.+){2,}/.test(email)) {
					$.ajax({
						//url: $this.attr('action'),/Epaper.asp?EMAIL=asdf%40asdf
						url: $this.attr('https://www.veryyou.com.tw/Epaper.asp'),
						type: 'POST',
						data: $('#form_epaper1').serialize(),
